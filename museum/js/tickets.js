...
function changeTicketsAmount(event){
    if(event.className == 'tickets__btn minus-basic' && inputBasic[0].value > 0){
        inputBasic[0].value--;
        inputBasic[1].value--;
    }
    else if(event.className == 'tickets__btn plus-basic' && inputBasic[0].value < 20){
        console.log(inputBasic[0].value, inputBasic[1].value);
        inputBasic[0].value++;
        inputBasic[1].value++;
    }
    else if(event.className == 'tickets__btn minus-senior' && inputSenior[0].value > 0){
        inputSenior[0].value--;
        inputSenior[1].value--;
    }
    else if(event.className == 'tickets__btn plus-senior' && inputSenior[0].value < 20){
        inputSenior[0].value++;
        inputSenior[1].value++;
    }
    tickets__total.innerHTML = `Total €${inputBasic[0].value * 20 + inputSenior[0].value * 10}`;
    overviewTotal.innerHTML = `${inputBasic[0].value * 20 + inputSenior[0].value * 10} €`;
